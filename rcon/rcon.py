...
class RCON(commands.Cog):
    ...
    async def remove(self, ctx):
        ...
        if str(ctx.author.id) in p_in_conf:
            ...
            await ctx.send(_("You are not whitelisted using RCON."))
    ...
    async def _send_pending_owner_notifications(self, bot):
        if await self.config.notification() == 0:
            await bot.send_to_owners(
                "RCON: With version 2.0.0 rcon has migrated to RCON. You will have to **set up your server connection**. More information available with `[p]whitelister setup`."
            )
